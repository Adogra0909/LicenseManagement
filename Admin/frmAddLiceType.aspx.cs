...
public partial class frmAddLiceType : System.Web.UI.Page
{
    ...
    private void FillLicenseType()
    {
        try
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("AMS_spLiceType_Master", con))
                {
                    con.Open();
    ...
    protected void btnTypeAdd_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@LicTypeCode", txtlicensetypecode.Text.Replace(" ", string.Empty));
                    cmd.Parameters.AddWithValue("@LicTypeName", txtlictypeName.Text);
    ...
    protected void gvstate_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "DeleteEx")
            {
                ...
                string LicTypeCode = gvstate.DataKeys[rowIndex].Values["LicTypeCode"].ToString();
                ...
                try
                {
                    using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
                    {
                        ...
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.AddWithValue("@LicTypeCode", LicTypeCode);
                            ...
                            lblsuccess.Text = LicTypeCode + " Deleted successfully";
            ...
            if (e.CommandName == "SelectState")
            {
                ...
                int rowIndex = Convert.ToInt32(e.CommandArgument);
                //Get the value of column from the DataKeys using the RowIndex.
                txtlicensetypecode.Text = gvstate.DataKeys[rowIndex].Values["LicTypeCode"].ToString();
                txtlictypeName.Text = gvstate.Rows[rowIndex].Cells[1].Text.Trim();
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@LicTypeCode", txtlicensetypecode.Text);
                    cmd.Parameters.AddWithValue("@LicTypeName", txtlictypeName.Text);
