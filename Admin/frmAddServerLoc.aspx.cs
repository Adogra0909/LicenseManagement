...
public partial class frmAddServerLoc : System.Web.UI.Page
{
    ...
    private void FillServerLoc()
    {
        try
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("AMS_spServerLoc_Master", con))
                {
                    con.Open();
    ...
    protected void btnTypeAdd_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ServerLocCode", txtserverlocCode.Text.Replace(" ", string.Empty));
                    cmd.Parameters.AddWithValue("@ServerLocName", txtserverlocName.Text);
    ...
    protected void gvstate_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "DeleteEx")
            {
                ...
                string ServerLocCode = gvstate.DataKeys[rowIndex].Values["ServerLocCode"].ToString();
                ...
                try
                {
                    using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
                    {
                        ...
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.AddWithValue("@ServerLocCode", ServerLocCode);
                            ...
                            lblsuccess.Text = ServerLocCode + " Deleted successfully";
            ...
            if (e.CommandName == "SelectState")
            {
                ...
                int rowIndex = Convert.ToInt32(e.CommandArgument);
                //Get the value of column from the DataKeys using the RowIndex.
                txtserverlocCode.Text = gvstate.DataKeys[rowIndex].Values["ServerLocCode"].ToString();
                txtserverlocName.Text = gvstate.Rows[rowIndex].Cells[1].Text.Trim();
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ServerLocCode", txtserverlocCode.Text);
                    cmd.Parameters.AddWithValue("@ServerLocName", txtserverlocName.Text);
